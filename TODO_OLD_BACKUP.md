...
## 📦 Complete Infrastructure Inventory (Day 1 - DONE)

**All infrastructure complete and tested** - Use these, don't recreate them!

#### 1. ✅ Current User + Session (COMPLETED)
- [x] Backend: `get_current_user()` command in `core/commands.rs`
- [x] Frontend: `useCurrentUser()` hook with TanStack Query
- [x] Display "Welcome {user}" in AppNavigation (right side)
- [x] Type: `CurrentUser` with username, home_dir, user_id
- **Why**: Stable identity for automation, personalization, future multi-user
- **Location**: `core/api/tauri.ts`, `core/hooks/useCurrentUser.ts`
- **Implementation**: Returns system username, home directory, and user ID (from $USER/$USERNAME and $HOME/$USERPROFILE env vars)

#### 2. ✅ Settings Registry (COMPLETED)
- [x] Backend: Typed settings schema with feature registration (already existed)
- [x] Frontend: `useSetting(key)`, `updateSetting(key, value)` hooks
- [x] Typed keys enum: `SettingKey.NewsDataApiKey`, `SettingKey.SyncInterval`, etc.
- [x] Settings per feature (news, google, reddit, x) with enable/disable
- **Why**: Prevents API key chaos, enables per-feed configuration
- **Location**: `core/settings/`, `shared/settings.ts`
- **Implementation**:
  - `shared/settings.ts`: SettingKey enum (21 settings), SettingValueType<K> generic, SETTINGS_REGISTRY
  - `core/hooks/useSettings.ts`: 6 typed hooks including useCapabilities() for feature flags
  - `core/api/tauri.ts`: AppSettingsDto grouped by category (general/news/writing/appearance/advanced)

#### 3. ✅ Query Keys Pattern (COMPLETED)
- [x] Create `shared/queryKeys.ts` with typed key factory
- [x] Helper functions: `invalidateIdeas()`, `invalidateIdea(id)`
- [x] Pattern: `['ideas']`, `['ideas', id]`, `['ideas', { status, priority }]`
- **Why**: Predictable cache invalidation, prevents stale data bugs
- **Location**: `shared/queryKeys.ts`
- **Implementation**:
  - `queryKeys` object with all domains (ideas, articles, sources, feeds, settings, logs, tasks, storage, currentUser)
  - `invalidation` helpers for common patterns
  - Type-safe with `as const` for readonly tuples
  - Updated useSettings() and useCurrentUser() to use centralized keys

#### 4. ✅ Feature Flags / Capabilities (COMPLETED)
- [x] Create `shared/capabilities.ts` with feature registry (integrated in useSettings)
- [x] Boolean flags: `news.enabled`, `google.enabled`, `reddit.enabled`, `x.enabled`
- [x] Check API key presence to auto-enable/disable features
- [x] UI: Hide nav items, show "not configured" states
- **Why**: Graceful degradation for unconfigured APIs
- **Location**: `shared/capabilities.ts`, `core/hooks/useCapabilities.ts`
- **Implementation**:
  - `core/hooks/useSettings.ts`: useCapabilities(), useFeatureEnabled()
  - `core/components/ui/CapabilityGate.tsx`: Conditional rendering component with fallback
  - `useCapabilityNav()`: Filter navigation items by feature availability

#### 5. ✅ Standard Loading/Error/Empty Components (COMPLETED)
- [x] `<LoadingState />` - Consistent spinners
- [x] `<EmptyState title description action />` - "No ideas yet" states
- [x] `<ErrorState message onRetry />` - Error boundaries per view
- **Why**: Prevents UI inconsistency, makes app feel polished
- **Location**: `core/components/ui/`
- **Implementation**:
  - `LoadingState.tsx`: LoadingState (full), LoadingInline (compact)
  - `ErrorState.tsx`: ErrorState with retry, ErrorInline (compact)
  - `EmptyState.tsx`: EmptyState with icon/action, EmptyInline (compact)
...
**Theme System**:
- ✅ `ThemeProvider` - Context with 3 themes (Dark/Light/Cyberpunk)
- ✅ `ThemeSwitcher` component - Dropdown with icons, ready to use
- ✅ `useTheme()` hook - `{ theme, setTheme }` with localStorage persistence
- ✅ CSS custom properties - Full theming system with CSS variables
- ✅ Radix UI integration - Maps to Radix appearance and accentColor

**State & Data Management**:
- ✅ TanStack Query - Smart retry, exponential backoff, stale time configured
- ✅ Query Keys Factory - `shared/queryKeys.ts` with typed hierarchical keys
- ✅ Zustand stores folder - Empty but ready for cross-view state
- ✅ Typed Tauri API - `core/api/tauri.ts` with all command wrappers

**UI Components**:
- ✅ Toast System - `core/lib/toast.ts` with `toast.success()`, `toast.error()`
- ✅ Dialog System - `useDialog()` with promise-based confirm/prompt
- ✅ Loading/Error/Empty - Consistent state components with variants
- ✅ Sidebar - `core/components/layout/Sidebar.tsx` with collapse persistence
- ✅ AppShell + AppNavigation - Top nav with domain switching

**Layout & Routing**:
- ✅ AppShell - Main layout structure with ErrorBoundary
- ✅ AppNavigation - Top horizontal nav with "Welcome, {user}"
- ✅ Sidebar - Per-domain navigation (writing/research/system)
- ✅ SystemView - Domain coordinator with <Outlet /> for sub-routes
- ✅ Routes - All System routes registered (Settings, Storage, Logs, Tasks)

**Developer Experience**:
- ✅ Path aliases - `@/core`, `@/shared`, `@/features`, `@/domains`
- ✅ TypeScript strict mode - Full type safety
- ✅ Vite config - Fast builds with HMR
- ✅ Directory structure - Clean 3-tier architecture

**Build Status**: ✅ Compiles successfully (308.22 kB bundle, 95.55 kB gzipped)

---

## ✅ COMPLETED: System Domain Infrastructure Fixes

**Status**: All 4 System domain components now using proper infrastructure  
**Fixed**: Settings, Storage, Logs, Tasks - All use custom toast, queryKeys, loading states  
**Build**: ✅ Compiles successfully (347.27 kB bundle)

### ✅ Settings.tsx - Complete Refactor

**Location**: `domains/system/Settings/Settings.tsx`

**Fixed Issues**:
1. ✅ Deleted local `useSettings.ts` hook (duplicate)
2. ✅ Uses type-safe `useSetting<K>()`, `useUpdateSetting<K>()` from core
3. ✅ Theme wired to `useTheme()` + backend persistence
4. ✅ Boolean values properly typed (not string conversion)
5. ✅ All labels from `SETTINGS_REGISTRY` metadata
6. ✅ Direct enum access via `SettingKey.*` instead of string patterns
7. ✅ Fixed infinite loading spinner bug (was checking `undefined`, now uses `isLoading`)

### ✅ Storage.tsx - Infrastructure Update

**Fixed Issues**:
1. ✅ Custom toast from `@/core/lib/toast` (not sonner)
2. ✅ Query keys from `queryKeys.storage.stats()`, `queryKeys.storage.backups()`
3. ✅ Loading states with `<LoadingState />` and `<LoadingState.Inline />`
4. ✅ Empty states with `<EmptyState.Inline />` for no backups
5. ✅ CSS custom properties for all colors

### ✅ Logs.tsx - Infrastructure Update

**Fixed Issues**:
1. ✅ Custom toast from `@/core/lib/toast` (not sonner)
2. ✅ Query keys from `queryKeys.logs.list()`, `queryKeys.logs.stats()`
3. ✅ Added `logs.stats()` method to queryKeys factory

### ✅ Tasks.tsx - Infrastructure Update

**Fixed Issues**:
1. ✅ Custom toast from `@/core/lib/toast` (not sonner)
2. ✅ Query keys from `queryKeys.tasks.list()`, `queryKeys.tasks.history()`
3. ✅ Added `tasks.history()` method to queryKeys factory

**Previous Problems** (Now Fixed):
1. ❌ Uses local `useSettings.ts` hook (duplicate) - DELETE THIS FILE
2. ❌ Imports `sonner` instead of custom toast
3. ❌ Theme shows placeholder text, doesn't use `ThemeSwitcher` component
4. ❌ Converts booleans to strings unnecessarily ('true'/'false')
5. ❌ Hardcodes labels/descriptions instead of using `SETTINGS_REGISTRY`
6. ❌ Manual category access pattern `settings?.category?.['key']?.value`

**Fix Actions**:
```tsx
// DELETE: domains/system/Settings/useSettings.ts

// UPDATE: domains/system/Settings/Settings.tsx
import { useSetting, useUpdateSetting } from '@/core/hooks/useSettings';
import { SettingKey, SETTINGS_REGISTRY, getSettingsByCategory } from '@/shared/settings';
import { toast } from '@/core/lib/toast';
import { useTheme } from '@/core/providers/ThemeProvider';
import { ThemeSwitcher } from '@/core/components/theme/ThemeSwitcher';

// General tab - Use typed hooks
const autoStart = useSetting(SettingKey.AppAutoStart); // boolean, not string
const updateSetting = useUpdateSetting();

const handleToggle = async (key: SettingKey, value: boolean) => {
  await updateSetting(key, value); // Auto-saves, no debounce needed
  toast.success('Setting updated');
};

// Appearance tab - Wire up ThemeSwitcher
const { theme, setTheme } = useTheme();
const updateThemeSetting = useUpdateSetting();

const handleThemeChange = async (newTheme: 'dark' | 'light' | 'cyberpunk') => {
  setTheme(newTheme); // Instant UI update
  await updateThemeSetting(SettingKey.AppTheme, newTheme); // Persist to backend
};

// Render ThemeSwitcher (already has all logic)
<ThemeSwitcher />

// OR use SETTINGS_REGISTRY to generate UI dynamically:
const generalSettings = getSettingsByCategory('general');
generalSettings.map(setting => {
  const value = useSetting(setting.key as SettingKey);
  // Render based on setting.type (boolean = Switch, string = TextField, etc.)
});
```

**Files to Update**:
1. DELETE `domains/system/Settings/useSettings.ts` (27 lines - duplicate)
2. UPDATE `domains/system/Settings/Settings.tsx` - Replace imports, use core infrastructure
3. UPDATE `domains/system/Storage/Storage.tsx` - Change `sonner` → `@/core/lib/toast`
4. UPDATE `domains/system/Logs/Logs.tsx` - Change `sonner` → `@/core/lib/toast`
5. UPDATE `domains/system/Tasks/Tasks.tsx` - Change `sonner` → `@/core/lib/toast`
