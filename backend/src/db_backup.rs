...
pub async fn backup_database(db_path: &str, backup_path: &str) -> AppResult<()> {
    if !Path::new(db_path).exists() {
        return Err(AppError::other(format!("Database file not found: {}", db_path)));
    }
...
pub async fn export_to_sql(db: &DatabaseConnection, output_path: &str) -> AppResult<()> {
    ...
    for table_row in tables {
        ...
        let create_result = db.query_one(Statement::from_string(
            db.get_database_backend(),
            create_sql,
        )).await?;
        ...
        if let Some(row) = create_result {
            ...
            let data_rows = db.query_all(Statement::from_string(
                db.get_database_backend(),
                data_sql,
            )).await?;
